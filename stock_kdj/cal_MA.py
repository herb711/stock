...
# 判断ma20和ma60是否有交叉
'''
def MA_cross(ma60, ma20):
    cross = []
    up = False
    down = False
    
    # 非法检查
    if len(ma60)<1 or len(ma20) <1 or len(ma60)!=len(ma20) :
        return cross
    ...
    for i in range(len(ma60)):
        # 判断有无交叉
        if ma60[i] > ma20[i] and up: # 本来在上，变成在下
            up = False
            down = True
            cross.append(i)# 记录
        elif ma60[i] < ma20[i] and down: # 本来在下，变成在上
            up = True
            down = False
            # 不记录
        ...
        if not down and not up:
            if ma60[i] > ma20[i]: # 本来在上，变成在下
                up = False
                down = True  
            elif ma60[i] < ma20[i]: # 本来在下，变成在上
                up = True
                down = False         
